...
class SMSEmailGateway:
    ...
    def connect_smtp(self):
        ...
        try:
            ...
            # Auto-detect connection type based on port
            if self.smtp_port == 465:
                print("🔒 Using SSL encryption (port 465)...")
                # SSL connection - requires SMTP_SSL, no starttls()
                self.server = smtplib.SMTP_SSL(self.smtp_server, self.smtp_port, timeout=30)
                print("✅ SSL connection established")
            else:
                print("🔗 Using TLS encryption (port 587)...")
                # TLS connection - use SMTP + starttls()
